...
-(float)brightness
{
    HSVType hsv = hsvWithUIColor(self);
    float brightness = hsv.v;
    return brightness;
}

+ (UIColor *)colorWithRGBAHexAndroid:(UInt32)hex
{
    int a = (hex >> 24) & 0xFF;
    int r = (hex >> 16) & 0xFF;
    int g = (hex >> 8) & 0xFF;
...
+ (UIColor *)colorWithHexStringAndroid:(NSString *)stringToConvert
{
    ...
    return [UIColor colorWithRGBAHexAndroid:hexNum];
}
...
+ (UIColor*)colorByTemperatureValue:(int)temperature
{
    ...
    if(temperature <= 66){
        green = temperature;
        green = 99.4708025861f * (float)log(green) - 161.1195681661f;
        
        if(green < 0){green = 0;}
        if(green > 255){green = 255;}
    }else{
        green = temperature - 60;
        green = 288.1221695283f  * (float)pow(green,-0.0755148492);
    ...
    if(temperature >= 66){
        ...
        if(temperature <= 19){
            ...
            blue = temperature - 10;
            blue = 138.5177312231f * (float)log(blue) - 305.0447927307f;
    ...
    return [UIColor colorWithRed:(int)red/255.0f green:(int)green/255.0f blue:(int)blue/255.0f alpha:1];
}
